...
from inventory_management_system_api.models.catalogue_item import CatalogueItemOut, PropertyOut
...
class ItemService:
    ...
    def create(self, item: ItemPostSchema) -> ItemOut:
        ...
        """
        catalogue_item = self._get_specified_catalogue_item(item.catalogue_item_id)

        self._get_specified_system(item.system_id)

        try:
            catalogue_category_id = catalogue_item.catalogue_category_id
            catalogue_category = self._get_specified_catalogue_category(catalogue_category_id)
        ...
        usage_status = self._get_specified_usage_status(usage_status_id)
    ...
    def get(self, item_id: str) -> ItemOut:
        """
        ...
        :raises InvalidObjectIdError: If the supplied `item_id` is invalid.
        """
        try:
            return self._item_repository.get(item_id)
        except (MissingRecordError, InvalidObjectIdError) as exc:
            exc.status_code = status.HTTP_404_NOT_FOUND
            exc.response_detail = "Item not found"
            raise exc
    ...
    def _get_specified_catalogue_item(self, catalogue_item_id: str) -> CatalogueItemOut:
        """
        Retrieve a specified catalogue item by its ID.

        :param catalogue_item_id: ID of the specified catalogue item to retrieve.
        ...
        try:
            ...
            exc.status_code = status.HTTP_422_UNPROCESSABLE_ENTITY
            exc.response_detail = "Specified catalogue item not found"
            raise exc

    def _get_specified_system(self, system_id: str) -> SystemOut:
        """
        Retrieve a specified system by its ID.

        :param system_id: ID of the specified catalogue item to retrieve.
        ...
        try:
            ...
            exc.status_code = status.HTTP_422_UNPROCESSABLE_ENTITY
            exc.response_detail = "Specified system not found"
            raise exc

    def _get_specified_usage_status(self, usage_status_id: str) -> CatalogueItemOut:
        """
        Retrieve a specified usage status by its ID.

        :param usage_status_id: ID of the specified catalogue item to retrieve.
        ...
        try:
            ...
            exc.status_code = status.HTTP_422_UNPROCESSABLE_ENTITY
            exc.response_detail = "Specified usage status not found"
            raise exc
